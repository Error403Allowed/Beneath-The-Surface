...
func _ready() -> void:
	$Card/VBox/RetryButton.pressed.connect(_on_retry_pressed)
...
func setup(depth: float, reason: String) -> void:
	...
	$Card/VBox/DeathReason.text = reason
	_animate_intro()

func _animate_intro() -> void:
	# Background flashes red then settles
	var bg = $Background
	bg.color = Color(0.3, 0.0, 0.0, 0.95)
	var tween = create_tween().set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
	tween.tween_property(bg, "color", Color(0.0, 0.0, 0.05, 0.95), 0.8)
	...
	tween2.tween_property(card, "modulate:a", 1.0, 0.5)
	tween2.parallel().tween_property(card, "position:y", card.position.y - 50.0, 0.5)

	# Title shakes
	var title = $Card/VBox/Title
	var shake = create_tween()
	for i in range(6):
		shake.tween_property(title, "position:x", 6.0 * (1 if i % 2 == 0 else -1), 0.05)
	shake.tween_property(title, "position:x", 0.0, 0.05)
